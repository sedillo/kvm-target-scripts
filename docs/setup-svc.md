...
### To create VGPUs
```
chmod +x ./scripts/create-vgpu.sh
bash -x ./scripts/create-vgpu.sh
```
If you see the error: "sh: echo: I/O error"
Make sure your aperture size in BIOS is correct.

### To destroy VGPUs

File (executable): /var/vm/scripts/destroy-vgpu.sh
```
chmod +x ./scripts/destroy-vgpu.sh
bash -x ./scripts/destroy-vgpu.sh
```
